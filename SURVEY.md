...
**Overall: renderers Done, integration assertions Partial.**
...
The common integration test parametrizes every installed renderer and checks
...
`DESIGN.md` is behind the code. OpenAI Agents SDK is listed as future work in
section 19, and Mastra is absent from sections 8, 17, and 19. Both are already
...
- **Inferred:** the comment says this schema is what the AI SDK serializes, but
  the worker does not call the AI SDK provider serializer.
...
- **Partial:** markdown includes J5 as a scorecard column but lacks the
  terminal reporter's dedicated J5 findings block.
- **Partial:** `--job` can filter J5 out through
  `src/mcp_mirror/report.py:57-67`, which conflicts with section 10's statement
  that J5 is mandatory in every report.

The scorecard computes an internal `worst` value in
`src/mcp_mirror/scorecard.py:129-138`. It is not presented as an overall winner,
but its name deserves review under the census-not-leaderboard rule.
...
- **Missing:** the section 17 launch writeup and the dated 2026-07-28 hook exist
  only in `DESIGN.md:350-354`, not in README.
...
**Overall: met at runtime, with evidence-quality gaps above.**
...
The definition of done does not prove every renderer reaches the final provider
request boundary. Where the implementation inspects an intermediate adapted
tool, this survey marks the stronger claim **Inferred**.

## Design decisions

- **D1, real adapters: Done.** All five implementations call published
  framework MCP adapters or clients. Provider-bound serialization strength
  varies by framework as described above.
...
- **D3, spec-version tagging: Done.** Negotiated version, report field,
  assertion, output header, and baseline gate are implemented. HTTP has no
  separate test.
- **D4, J5 first-class: Done with presentation gaps.** Annotation fate,
  risk-language loss, J5 aggregation, tests, and terminal findings are present.
  J5 remains filterable and markdown lacks the dedicated findings section.
...
1. Add HTTP source-loader and CLI exit-code tests.
2. Replace the deprecated Pydantic AI MCP entry point before v2.
3. Add per-renderer golden fixture expectations across J1-J5 dimensions.
4. Capture or verify the final provider-bound request for renderers currently
   marked **Inferred**.
5. Decide whether J5 must remain visible when `--job` filters are supplied.
6. Decide whether to retain the internal `worst` aggregate.
7. Update `DESIGN.md` sections 6, 8, 9, 17, and 19 after those decisions.
